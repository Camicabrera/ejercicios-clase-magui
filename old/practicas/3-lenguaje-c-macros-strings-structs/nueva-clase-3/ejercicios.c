...
char* mi_strdup(const char* str) {
    ...
    char * dup = (char *) malloc(sizeof(char) * (mi_strlen(str) + 1));
...
char* mi_strconcat(const char* s1, const char* s2) {
    ...
    char * concat = malloc(sizeof(char)*(mi_strlen(s1)+ mi_strlen(s2) + 1));

    mi_strcpy(concat, s1);
    mi_strcpy(concat + mi_strlen(s1), s2);
...
Contacto* crear_contacto(const char* nombre, const char* telefono, const char* email) {
    ...
    contacto -> nombre = mi_strdup(nombre);
    contacto -> telefono = mi_strdup(telefono);
    contacto -> email = mi_strdup(email);
...
Agenda* crear_agenda(int capacidad_inicial) {
    ...
    agenda -> cantidad = 0;
    agenda -> capacidad = capacidad_inicial;
    agenda -> contactos = malloc(sizeof(Contacto*) * capacidad_inicial);
    return agenda;
}
