...
class MyDictionary:
    ...
    def insert(self, key, value):
        ...
        for _ in range(self.capacity):
            ...
            if(node and node.key != None and node.key == key):
                node.value = value
                return True
        
            if(node == None or node.was_deleted == True):
    ...
    def get_by_key(self, key):
        ...
        for _ in range(self.capacity):
            ...
            if(node and node.key == key and node.was_deleted == False):
                return node.value
    ...
    def delete_by_key(self, key):
        ...
        for _ in range(self.capacity):
            ...
            if(node and node.key != None and node.key == key and node.was_deleted == False):
                node.value = None
                node.key = None
